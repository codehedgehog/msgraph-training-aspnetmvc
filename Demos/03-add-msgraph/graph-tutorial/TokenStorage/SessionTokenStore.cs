...
namespace graph_tutorial.TokenStorage
{
	...
	public class SessionTokenStore
	{
		...
		private string GetUsersUniqueId(ClaimsPrincipal user)
		{
			...
			if (user != null)
			{
				var userObjectId = user.FindFirst("http://schemas.microsoft.com/identity/claims/objectidentifier").Value ?? user.FindFirst("oid").Value;
				var userTenantId = user.FindFirst("http://schemas.microsoft.com/identity/claims/tenantid").Value ?? user.FindFirst("tid").Value;
